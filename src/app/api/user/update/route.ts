...
export async function POST(request: Request) {
  try {
    ...
    const {username, email, bio, firstName, lastName, pic} = User.parse(credentials)
    if (username !== old) {
      const user = await users?.findOne({username})
      if (user) return NextResponse.json({ msg: "Username already exists" }, {status: 409})
    }
    if (email !== oldEmail) {
      const user = await users?.findOne({email})
      if (user) return NextResponse.json({ msg: "Email already exists" }, {status: 409})
    }
    const user = await users?.updateOne({username: old}, { $set: {bio, firstName, lastName, avatar: pic, username, email } })
    if (!user) return NextResponse.json( {msg: "Profile update failed" }, {status: 409})
    if (!user) return NextResponse.json( {msg: "Profile update failed" }, {status: 409})
